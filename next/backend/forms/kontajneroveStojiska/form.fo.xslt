...
<xsl:stylesheet xml:lang="en" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="map">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$template = 'ziadatel_udaje'">
        <xsl:call-template name="ziadatel_udaje">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'typ_stanovista'">
        <xsl:call-template name="typ_stanovista">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'stanoviste_informacie'">
        <xsl:call-template name="stanoviste_informacie">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'miesto_umiestnenia'">
        <xsl:call-template name="miesto_umiestnenia">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'dovod_ziadosti'">
  ...
  <xsl:template name="ziadatel"><xsl:param name="values" /><xsl:if test="$values/z:ZiatetelTyp"><xsl:call-template
        ...
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:ZiatetelTyp" />
  ...
  <xsl:template name="ziadatel_udaje"><xsl:param name="values" /><xsl:if test="$values/z:Nazov"><xsl:call-template
        name="base_labeled_field">
      ...
      test="$values/z:Email"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'E-mail'" />
        <xsl:with-param name="node" select="$values/z:Email" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:Telefon"><xsl:call-template name="base_labeled_field">
  ...
  <xsl:template name="typ_stanovista"><xsl:param name="values" /><xsl:if
      test="$values/z:TypStanovistaUpresnenie"><xsl:call-template name="base_labeled_field">
  ...
  <xsl:template name="stanoviste_informacie"><xsl:param name="values" /><xsl:if
      test="$values/z:MestomPonukanaDokumentacia"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Použijete niektorú z mestom ponúkaných projektových dokumentácií?'" />
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:MestomPonukanaDokumentacia" />
          </xsl:call-template></xsl:with-param>
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:PopisZbernychNadob"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Popíšte počet a typ jednotlivých zberných nádob a ich objemy'" />
        <xsl:with-param name="node" select="$values/z:PopisZbernychNadob" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:VegetacnaStrecha"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Plánujete vybudovať / máte vegetačnú strechu?'" />
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:VegetacnaStrecha" />
          </xsl:call-template></xsl:with-param>
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:VegetacnaStena"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Plánujete vybudovať / máte vegetačnú stenu kontajnerového stanovišťa?'" />
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:VegetacnaStena" />
          </xsl:call-template></xsl:with-param>
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:SadoveUpravy"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Plánujete / urobili ste sadové úpravy bezprostredného okolia?'" />
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:SadoveUpravy" />
          </xsl:call-template></xsl:with-param>
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="miesto_umiestnenia"><xsl:param name="values" /><xsl:for-each
      select="$values/z:Kataster">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Katasttrálne územie pozemku'" />
        <xsl:with-param name="node" select="." />
      </xsl:call-template>
    </xsl:for-each><xsl:if
      test="$values/z:Parcela"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Parcelné číslo pozemku'" />
        <xsl:with-param name="node" select="$values/z:Parcela/z:Name" />
      </xsl:call-template></xsl:if><xsl:for-each
      select="$values/z:AdresaUmiestnenia">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Adresa bytových domov, ktorým bude stanovište slúžiť'" />
        <xsl:with-param name="node" select="." />
      </xsl:call-template>
    </xsl:for-each><xsl:if
      test="$values/z:Velkost"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Veľkosť plochy (v m2)'" />
        <xsl:with-param name="node" select="$values/z:Velkost" />
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="dovod_ziadosti"><xsl:param name="values" /><xsl:if test="$values/z:Zaujem"><xsl:call-template
        name="base_labeled_field">
        <xsl:with-param name="text" select="'Mám záujem o'" />
        <xsl:with-param name="node" select="$values/z:Zaujem" />
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="technicky_vykres"><xsl:param name="values" /><xsl:for-each
      select="$values/z:TechnickyVykresPrilohy">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Prílohy'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:if
      test="$values/z:TechnickyVykresText"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Text'" />
        <xsl:with-param name="node" select="$values/z:TechnickyVykresText" />
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="rozpocet"><xsl:param name="values" /><xsl:for-each
      select="$values/z:RozpocetPrilohy">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Prílohy'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:if
      test="$values/z:RozpocetText"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Text'" />
        <xsl:with-param name="node" select="$values/z:RozpocetText" />
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="wrapper"><xsl:param name="values" /><xsl:for-each select="$values/z:Zmluva">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Zmluva o výkone správy / Zmluva o bytovom spoločenstve'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:SnimkaZMapy">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Snímka z katastrálnej mapy so zakresleným kontajnerovým stanovišťom'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:PodorysStanovista">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Pôdorys kontajnerového stanovišťa so zakreslením rozmiestnenia zberných nádob'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:VizualStanovista">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Vizuál kontajnerového stanovišťa - ilustračná alebo reálna fotografia / vizualizácia'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:call-template
      name="technicky_vykres">
      <xsl:with-param name="values" select="$values/*[local-name() = 'TechnickyVykres']" />
    </xsl:call-template><xsl:for-each
      select="$values/z:StavebnaDokumentacia">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Stavebná dokumentácia kontajnerového stanovišťa alebo výber z poskytnutých stavebných dokumentácií'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:call-template
      name="rozpocet">
      ...
      select="$values/z:InePrilohy">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Iné'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:Stanovisko">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Stanovisko OLO'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:Doklad">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Relevantný doklad potvrdzujúci súčasný odvoz'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:for-each
      select="$values/z:ListVlastnictva">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text"
          select="'Zmluva o nájme pozemku / List vlastníctva / Zmluva o zriadení vecného bremena'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each></xsl:template>
</xsl:stylesheet>