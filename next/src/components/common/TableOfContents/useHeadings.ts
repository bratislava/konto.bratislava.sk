...
import { PAGE_CONTENT_ID } from '@/src/components/page-contents/FormLandingPageContent/FormLandingPageContent'
...
const useHeadings = () => {
  ...
  const getQueryList = useCallback((headingLevels: HeadingLevels[]) => {
    ...
    const queryList = headingLevels
      .map((level) => `#${PAGE_CONTENT_ID} div[data-toc=${ATTRIBUTE_TABLE_OF_CONTENTS}] ${level}`)
      .join(', ')
    ...
    const queryListHeadings = headingLevels
      .map(
        (level) =>
          `#${PAGE_CONTENT_ID} div[data-toc=${ATTRIBUTE_TABLE_OF_CONTENTS_HEADING}] ${level}`,
      )
      .join(', ')
  ...
  useEffect(() => {
    if (isClient) {
      ...
      const rootElement = document.querySelector(`#${PAGE_CONTENT_ID}`)
      if (rootElement) {
