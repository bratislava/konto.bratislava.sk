...
export default class FormsService {
  ...
  async archiveForm(
    ...
    if (form.state !== FormState.DRAFT) {
      throw this.throwerErrorGuard.BadRequestException(
        FormsErrorsEnum.FORM_NOT_DRAFT_ERROR,
        FormsErrorsResponseEnum.FORM_NOT_DRAFT_ERROR,
      )
    }
  ...
  async getForms(
    ...
    const { formName, pospID, states, schemaVersionId } = query
  ...
  async checkFormBeforeSending(id: string): Promise<FormWithSchemaAndVersion> {
    ...
    if (form.state !== FormState.DRAFT) {
      throw this.throwerErrorGuard.UnprocessableEntityException(
        FormsErrorsEnum.FORM_NOT_DRAFT_ERROR,
        `${FormsErrorsResponseEnum.FORM_NOT_DRAFT_ERROR} Current form state is: ${form.state}.`,
      )
    }
    return form
  }
