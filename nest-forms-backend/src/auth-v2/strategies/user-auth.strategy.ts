...
export class UserAuthStrategy extends PassportStrategy(
  ...
  constructor(
    ...
    private readonly cognitoAttributesService: CognitoUserService,
  ...
  private async authenticateWithBearerToken(bearerToken: string) {
    ...
    const [cognitoAttributes, cityAccountUser] = await Promise.all([
      this.cognitoAttributesService.getUserAttributes(cognitoJwtPayload.sub),
  ...
  async validate(req: Request): Promise<AuthUser | GuestUser> {
