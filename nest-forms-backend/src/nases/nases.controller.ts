...
export default class NasesController {
  ...
  @ApiBadRequestResponse({
    ...
    schema: {
      anyOf: [
        {
          $ref: getSchemaPath(FormNotDraftErrorDto),
        },
      ],
    },
  })
  ...
  async updateFormEid(
    ...
    const returnData = await this.nasesService.updateFormEid(id, user, data)
    return returnData
  }
  ...
  @ApiExtraModels(FormNotDraftErrorDto)
  @ApiExtraModels(FormNotFoundErrorDto)
  @ApiResponse({
    status: 404,
    description: 'Not found error.',
  ...
  async sendFormEid(
    ...
    const jwt = this.nasesUtilsService.createUserJwtToken(body.eidToken)
  ...
  async sendAndUpdateFormEid(
    ...
    await this.nasesService.updateFormEid(id, user, updateData)
