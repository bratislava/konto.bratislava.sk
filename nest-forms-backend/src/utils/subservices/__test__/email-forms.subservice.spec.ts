...
describe('EmailFormsSubservice', () => {
  ...
  describe('transcode', () => {
    ...
    it('should process a valid email form successfully', async () => {
      ...
      await service.transcode(mockJob as Job<{ formId: string }>)
    ...
    it('should throw NotFoundException when form is not found', async () => {
      ...
      await expect(
        service.transcode(mockJob as Job<{ formId: string }>),
      ).rejects.toThrow(
  ...
  describe('handler', () => {
    it('should update form error state when job fails', async () => {
      ...
      service.handler(mockJob as Job<{ formId: string }>, mockError)

      expect(prismaMock.forms.update).toHaveBeenCalledWith({
        where: { id: 'test-form-id' },
        data: { error: FormError.EMAIL_SEND_ERROR },
      })
    })
  })
  ...
  describe('getFileIdsToUrlMap', () => {
    it('should return correct file URL map', () => {
      ...
      expect(result.file1).toMatch(/^http:\/\/frontend\.com\/download\/file\//)
      expect(result.file2).toMatch(/^http:\/\/frontend\.com\/download\/file\//)
    })
  })
