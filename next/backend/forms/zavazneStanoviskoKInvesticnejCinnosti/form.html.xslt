...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:z="http://schemas.gov.sk/doc/eform/00603481.zavazneStanoviskoKInvesticnejCinnosti.sk/0.1"
  ...
  <xsl:template name="map">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$template = 'investor'">
        <xsl:call-template name="investor">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'projektant'">
        <xsl:call-template name="projektant">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'stavba'">
        <xsl:call-template name="stavba">
          <xsl:with-param name="values" select="$values" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$template = 'konanie'">
  ...
  <xsl:template name="base_title">
    ...
    <div class="caption ui-widget-header"
      onmousedown="collapse_section(this)">
      <div class="headercorrection">
  ...
  <xsl:template name="base_labeled_field">
    ...
    <xsl:variable
      name="divStyle"
      select="concat('display: table-cell; vertical-align: middle; width: ', $width + 10, 'px;')" />
    <xsl:variable
      name="inputStyle" select="concat('width: ', $width , 'px')" />
    ...
    <div class="cell">
      ...
      <span class="fieldContent" style="width: 100%">
        <div style="padding: 0 10px 0 0;">
          <textarea rows="6" class="textarea-color" disabled="disabled"
            style="resize: none; width: 100%; margin: 0; padding: 0;">
            <xsl:value-of select="$node" />
          </textarea>
        </div>
      </span>
  ...
  <xsl:template name="base_custom_css"> label.socialStatusLabel { display: inline-block; font-size:
    0.75em; line-height: 25px; margin: 0px 18px 0px 0px; padding: 0; width: 250px; }
    label.checkboxLabel { font-size: 0.75em; margin: 0px; padding: 0; } div.checkbox { padding-left:
    20px; } div.checkbox input[type="checkbox"] { margin: 0px 5px 0px -20px; vertical-align:middle;
    } div.radiobutton { padding-left: 20px; } div.radiobuttonHorizontal { padding-right: 20px;
    float: left; } div.radiobutton input[type="radio"] { margin: 0px 5px 0px -20px;
    vertical-align:middle; } input.iban { width:200px; } .col33 { width: 33%; } .col40 { width: 40%;
    } .col50 { width: 50%; } .col60 { width: 60%; } .col75 { width: 75%; } .cell { overflow: auto; }
    p { margin: 10px 10px 0px 10px; font-size: 0.75em; } table { border-collapse: collapse; } table,
    td, th { border: 1px solid #C7C7C5; font-size: 0.9em; padding: 3px; } ul { margin: 0px 0px 0px
    0px; font-size: 0.75em; } li { margin: 0px 0px 0px 0px; } .table { width: 100%; }
    .textarea-color[disabled="disabled"] { color: rgb(112, 112, 112) !important; } <!-- *[disabled="disabled"] { color : black !important; } -->
  </xsl:template>

  <xsl:template name="base_default_css">
    <link href="https://www.slovensko.sk/static/eForm/Designer/2.0.1.12/Styles/jquery-ui.css"
      rel="stylesheet" type="text/css" />
    <link
      href="https://www.slovensko.sk/static/eForm/Designer/2.0.1.12/Styles/base.css"
      rel="stylesheet" type="text/css" />
    <link
      href="https://www.slovensko.sk/static/eForm/Designer/2.0.1.12/Styles/ego.css" rel="stylesheet"
      type="text/css" />
    <link
  ...
  <xsl:template name="prilohy"><xsl:param name="values" /><xsl:for-each
      select="$values/z:ProjektovaDokumentacia">
  ...
  <xsl:template name="ziadatel_mesto_psc"><xsl:param name="values" /><xsl:if
      test="$values/z:ZiadatelMesto"><xsl:call-template name="base_labeled_field">
      ...
      test="$values/z:ZiadatelPsc"><xsl:call-template name="base_labeled_field">
  ...
  <xsl:template name="investor"><xsl:param name="values" /><xsl:if
      test="$values/z:InvestorZiadatelom"><xsl:call-template name="base_labeled_field">
        ...
        <xsl:with-param name="node"><xsl:call-template name="base_boolean">
            <xsl:with-param name="bool" select="$values/z:InvestorZiadatelom" />
          </xsl:call-template></xsl:with-param>
      </xsl:call-template></xsl:if><xsl:for-each
      select="$values/z:Splnomocnenie">
      <xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Splnomocnenie na zastupovanie'" />
        <xsl:with-param name="node" select="z:Nazov" />
      </xsl:call-template>
    </xsl:for-each><xsl:if
      test="$values/z:InvestorTyp"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Typ investora'" />
        <xsl:with-param name="node" select="$values/z:InvestorTyp" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorMenoPriezvisko"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Meno a priezvisko'" />
        <xsl:with-param name="node" select="$values/z:InvestorMenoPriezvisko" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorObchodneMeno"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Obchodné meno'" />
        <xsl:with-param name="node" select="$values/z:InvestorObchodneMeno" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorIco"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'IČO'" />
        <xsl:with-param name="node" select="$values/z:InvestorIco" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorAdresaPobytu"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Adresa trvalého pobytu'" />
        <xsl:with-param name="node" select="$values/z:InvestorAdresaPobytu" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorMiestoPodnikania"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Miesto podnikania'" />
        <xsl:with-param name="node" select="$values/z:InvestorMiestoPodnikania" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorAdresaSidla"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Adresa sídla'" />
        <xsl:with-param name="node" select="$values/z:InvestorAdresaSidla" />
      </xsl:call-template></xsl:if><xsl:call-template
      name="investor_mesto_psc">
      <xsl:with-param name="values" select="$values/*[local-name() = 'InvestorMestoPsc']" />
    </xsl:call-template><xsl:if
      test="$values/z:InvestorKontaktnaOsoba"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Kontaktná osoba'" />
        <xsl:with-param name="node" select="$values/z:InvestorKontaktnaOsoba" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorEmail"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'E-mail'" />
        <xsl:with-param name="node" select="$values/z:InvestorEmail" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:InvestorTelefon"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Telefónne číslo (v tvare +421...)'" />
        <xsl:with-param name="node" select="$values/z:InvestorTelefon" />
      </xsl:call-template></xsl:if></xsl:template>
  <xsl:template name="projektant"><xsl:param name="values" /><xsl:if
      test="$values/z:ProjektantMenoPriezvisko"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Meno a priezvisko'" />
        <xsl:with-param name="node" select="$values/z:ProjektantMenoPriezvisko" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:ProjektantEmail"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'E-mail'" />
        <xsl:with-param name="node" select="$values/z:ProjektantEmail" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:ProjektantTelefon"><xsl:call-template name="base_labeled_field">
        <xsl:with-param name="text" select="'Telefónne číslo (v tvare +421...)'" />
        <xsl:with-param name="node" select="$values/z:ProjektantTelefon" />
      </xsl:call-template></xsl:if><xsl:if
      test="$values/z:AutorizacneOsvedcenie"><xsl:call-template name="base_labeled_field">
