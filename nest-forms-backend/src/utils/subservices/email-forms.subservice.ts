...
export default class EmailFormsSubservice {
  ...
  async transcode(job: Job<{ formId: string }>): Promise<void> {
    ...
    this.logger.log(
      `Sending email of form ${job.data.formId} to ${formDefinition.email}`,
    )
    ...
    try {
      ...
      alertError(
        `Sending confirmation email to ${toEmail} for form ${job.data.formId} failed.`,
        this.logger,
  ...
  handler(job: Job<{ formId: string }>, err: Error): void {
    alertError(
      `Sending email of form ${job.data.formId} has failed.`,
      this.logger,
      JSON.stringify(err),
    )
    ...
    this.prismaService.forms
      ...
      .catch((error) => {
        alertError(
          `Setting form error with id ${job.data.formId} to EMAIL_SEND_ERROR failed.`,
          this.logger,
          JSON.stringify(error),
        )
      })
