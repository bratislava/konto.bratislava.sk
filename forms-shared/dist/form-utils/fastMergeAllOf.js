...
exports.baFastMergeAllOf = void 0;
const validateObject = (schema, context) => {
    if (typeof schema !== 'object') {
        throw new Error(`${context} must be an object`);
    }
};
const validateSchemaType = (type, context) => {
    if (type && type !== 'object') {
        throw new Error(`${context} type must be either undefined or "object"`);
    }
};
const validateAllowedProperties = (schema, allowedProps, context) => {
    const invalidProps = Object.keys(schema).filter((prop) => !allowedProps.includes(prop));
 ...
 */
function baFastMergeAllOf(schema) {
...
exports.baFastMergeAllOf = baFastMergeAllOf;
