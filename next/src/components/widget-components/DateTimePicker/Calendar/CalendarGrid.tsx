...
const CalendarGrid = ({ state, offset = {}, ...rest }: CalendarGridBase) => {
  ...
  return (
    <div {...gridProps} className="flex flex-col items-center">
      <div
        {...headerProps}
        className="flex w-full justify-between border-y-2 border-gray-700 bg-gray-50 p-3 text-p-tiny-r font-medium lg:text-p-tiny"
      >
      ...
      <div className="flex w-full flex-col px-3 py-4">
        {weeksInMonthArr.map((weekIndex: number) => (
          <div className="mb-1 flex justify-between last:mb-0 sm:mb-0" key={weekIndex}>
            {state
