...
export const tsbExtractName = (formData: GenericObjectType) => {
  if (formData.objednavatel) {
    if (formData.objednavatel.objednavatelTyp === 'Fyzická osoba' || formData.objednavatel.objednavatelTyp === 'Fyzická osoba - podnikateľ') {
      return safeString(formData.objednavatel.menoPriezvisko?.meno)
    }
    if (
      formData.objednavatel.objednavatelTyp === 'Právnická osoba'
    ) {
