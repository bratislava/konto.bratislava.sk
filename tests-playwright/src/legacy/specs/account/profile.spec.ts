...
import { continueToAccount, openProfile, registerAccount } from '../../pages/AccountPage'
...
test.beforeEach(async ({ page, identity }) => {
  // Leaves the browser on the post-registration success screen, already signed in.
...
test('first and last name changes are saved', { tag: '@legacy' }, async ({ page, identity }) => {
  const meno = `Zmenene${identity.email.split('@')[0].slice(-6)}`
  const priezvisko = `Priezvisko${identity.email.split('@')[0].slice(-6)}`
