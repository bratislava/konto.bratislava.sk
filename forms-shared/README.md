...
```bash
pnpm run generate-slovensko-sk
```
...
If you are having problems with pnpm installation, you likely need to relink brew libraries:
...
On Windows just use pnpm install as the binaries are included in the packages:
...
And then proceed to pnpm install as usual:
...
You can run the tests using the following pnpm scripts:
