      ...
      context(device, Cypress.env('resolution')[`${device}`], () => {
        ...
        it('2. Editing personal information.', () => {
          ...
          cy.dataCy('edit-personal-information-form-container').then((form) => {
            ...
            cy.wrap(Cypress.$('[data-cy=input-phone_number]', form)).clear().type(this.fileData.wrong_phone_number)
            cy.wrap(Cypress.$('[data-cy=input-street_address]', form)).clear().type(this.fileData.address)
            cy.wrap(Cypress.$('[data-cy=input-city]', form)).clear().type(this.fileData.city)
            cy.wrap(Cypress.$('[data-cy=input-postal_code]', form)).clear().type(this.fileData.wrong_postal_code)
          })
          if (device === 'desktop') {
          ...
          cy.dataCy('edit-personal-information-form-container').then((form) => {
            ...
            cy.wrap(Cypress.$('[data-cy=error-message]', form)).should('contain', 'PSČ musí obsahovať práve 5 číslic.')
            
            cy.wrap(Cypress.$('[data-cy=input-postal_code]', form)).should('have.class', 'border-negative-700')
            cy.wrap(Cypress.$('[data-cy=input-postal_code]', form)).clear().type(this.fileData.postal_code)
          })
          if (device === 'desktop') {
            cy.get('[data-cy=save-personal-information-button]').click()
          } else {
