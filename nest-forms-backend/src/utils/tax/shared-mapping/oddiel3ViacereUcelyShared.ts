...
const getVymeryStaviebPodlaTypu = (
  ...
  const stavbyCalculated = safeArray(stavba.nehnutelnosti?.nehnutelnosti).map(
    (nehnutelnost) => {
      if (!pouzitKalkulacku) {
        return {
...
const mapPriznanie = (
  data: TaxFormData,
  priznanie: DanZoStaviebViacereUcelyPriznania,
) => {
  ...
  const vymeryStaviebPodlaTypu = getVymeryStaviebPodlaTypu(
    priznanie,
    pouzitKalkulacku,
  )
