...
const useGetContext = () => {
  ...
  const getValidatedSummary = useCallback(
    (precalculate?: boolean) => {
      // This function should be absolutely never called on screen containing a form, it would mean the summary is computed
      // for every form data change. Only exception is when called by `precalculateSummary`.
      if (
