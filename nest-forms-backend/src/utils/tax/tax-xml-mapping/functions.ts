...
import { formatDate } from '../shared-mapping/dates'

/**
 * Converts a Date object to a xs:date (e.g. `2021-01-01+01:00`) formatted string.
 */
export const formatXsDateXml = (date: Date | undefined) =>
  formatDate(date, 'YYYY-MM-DDZ')
...
const medzinarodneVolacieCisloRegex =
  /(((\+[1-9])|(\+[1-9]\d{1,8}))|((00[1-9])|(00[1-9]\d{1,7})))/ // TelefonneCisloType.MedzinarodneVolacieCislo in XSD
...
export const phoneNumberXml = (value: string | undefined | null) => {
  ...
  const [medzinarodneVolacieCislo, predvolba, ...rest] =
    internationalWithoutSlashes.split(' ')
  ...
  const newPhoneNumber = parsePhoneNumber(
    `${medzinarodneVolacieCislo}${predvolba}${cislo}`,
  )
  if (
    !newPhoneNumber ||
