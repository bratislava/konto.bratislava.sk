...
> [!NOTE]
> `clamav` and `cvdmirror` are outside this mechanism. Both are configured entirely by conf files baked into their images (`clamav/conf/`, `cvdmirror/Caddyfile`), so neither has an `-env` config map and the infrastructure repo reads no `.env.deploy.*` file for them — adding one here would be silently ignored until the wiring is added in the infrastructure repo first. This does not apply to `nest-clamav-scanner`, the API in front of clamav, which is a normal service and does use env files.
