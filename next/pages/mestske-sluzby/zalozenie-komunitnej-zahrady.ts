...
import zalozenieKomunitnejZahrady from 'schema-generator/definitions/zalozenieKomunitnejZahrady'
...
/**
 * Temporary route for previewing the community garden registration form.
 */
export const getServerSideProps: GetServerSideProps<FormPageWrapperProps, Params> = async (ctx) => {
  ...
  const schema = JSON.parse(JSON.stringify(zalozenieKomunitnejZahrady)) as {
    schema: RJSFSchema
    uiSchema: UiSchema
  }
