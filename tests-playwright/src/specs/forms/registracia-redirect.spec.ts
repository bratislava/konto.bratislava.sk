...
import { continueButton, fieldRoot } from '../../engine/selectors'
...
const ziadatelInput = (page: Page, property: string) =>
  fieldRoot(page, `root_ziadatel_${property}`).locator('input').first()
 ...
 */
test('rozpracovaný formulár vedie na registráciu a registrácia prebehne', async ({
  page,
  identity,
}) => {
