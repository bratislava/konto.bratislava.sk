...
export class BloomreachService {
  ...
  async sendDeliveryMethodChangedToNotify(cognitoId: string, birthNumber: string) {
    if (process.env.BLOOMREACH_INTEGRATION_STATE !== 'ACTIVE') {
      return
    }
    ...
    const pdf = await this.pdfConverterService.getPdfByTemplateName(
      ...
      'Súhlas so zasielaním oznámení.pdf',
      { firstName: user.family_name, email: user.email, birthNumber: birthNumber },
      birthNumber
    )

    try {
      const response = await axios.post(
        `${process.env.BLOOMREACH_API_URL}/email/v2/projects/${process.env.BLOOMREACH_PROJECT_TOKEN}/sync`,
        JSON.stringify({
          //
          integration_id: process.env.BLOOM_REACH_MAILGUN_INTEGRATION_ID,
          email_content: {
            template_id: process.env.BLOOMREACH_TEMPLATE_ID_DELIVERY_METHOD_SET_TO_NOTIFICATION,
            attachments: {
      ...
      if (response.status !== 200) {
        this.logger.error({
          message: 'Could not send delivery-method-set-to-notification email.',
          bloomreachResponse: response,
        })
      }
    } catch (error) {
      throw this.throwerErrorGuard.InternalServerErrorException(
        ErrorsEnum.INTERNAL_SERVER_ERROR,
        'Bloomreach service error.',
        `Bloomreach authentication/permission error for user ${cognitoId}`,
        error
      )
    }
  }
