...
export function createSwaggerDocument(app: INestApplication, overridePort?: number) {
  const port = overridePort ?? process.env.PORT ?? 3000
...
export async function bootstrap() {
  ...
  const document = createSwaggerDocument(app, Number(port))
