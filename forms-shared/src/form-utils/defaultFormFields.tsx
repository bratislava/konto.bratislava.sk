...
const ExtractUiSchemaSchemaField = (props: FieldProps) => {
  const isRoot = getObjectFieldInfo(props.idSchema)
  const baUiSchema = props.schema.baUiSchema

  if (isRoot && baUiSchema) {
    throw new Error('baUiSchema should not be present in the root schema')
  }

  if (isRoot && props.uiSchema === defaultUiSchema) {
