...
describe('OAuth2ExceptionFilter', () => {
  ...
  describe('catch - Authorization Endpoint Errors (RFC 6749 Section 4.1.2.1)', () => {
    ...
    it('should handle OAuth2Exception with metadata', () => {
      // CUSTOM PROXY DETAIL: OAuth2Exception can carry internal metadata
  ...
  describe('Error Code Mapping (RFC 6749 Compliance)', () => {
    ...
    it('should map 400 Bad Request to invalid_request for authorization endpoint', () => {
      // RFC 6749 Section 4.1.2.1: invalid_request for malformed or missing parameters
    ...
    it('should map 401/403 to access_denied for authorization endpoint', () => {
      // RFC 6749 Section 4.1.2.1: access_denied when resource owner denies the request
    ...
    it('should map 500 to server_error for authorization endpoint', () => {
      // RFC 6749 Section 4.1.2.1: server_error for unexpected conditions
    ...
    it('should map 504 Gateway Timeout to temporarily_unavailable', () => {
      // RFC 6749 Section 4.1.2.1: temporarily_unavailable for temporary overload/maintenance
  ...
  describe('Token Error Code Mapping', () => {
    ...
    it('should map 400 to invalid_request for token endpoint', () => {
      // RFC 6749 Section 5.2: invalid_request for malformed token requests
      const exception = new HttpException('Bad request', HttpStatus.BAD_REQUEST)
    ...
    it('should map 403 to unauthorized_client for token endpoint', () => {
      // RFC 6749 Section 5.2: unauthorized_client for clients not authorized to use this grant type
      const exception = new HttpException('Forbidden', HttpStatus.FORBIDDEN)
  ...
  /**
   * CUSTOM PROXY DETAIL: Internal Endpoints (/store and /info)
   *
   * These are custom endpoints for the "token staging" bridge pattern of our Cognito
   * OAuth2 proxy, described in sequence-diagram.md (lines 73-84).
   *
   ...
   * Not part of any RFC; they exist only inside our proxy boundary.
   */
  describe('Custom Proxy Detail - Internal Endpoints', () => {
  ...
  /**
   * CUSTOM PROXY DETAIL: extractRedirectUriAndStateFromQuery Method
   *
   ...
   *
   * Although the helper itself is custom, the validation it performs maps directly to
   * RFC 6749 Section 4.1.2.1 (verify the redirect URI is registered before redirecting
   * with an error) and RFC 6819 Section 4.4.1.7 (open redirector mitigation).
   *
  ...
  /**
   * CUSTOM PROXY DETAIL: State Mismatch Detection and Warning
   *
   * Internal sanity check inside our proxy: detects when an error response carries
   * a different state than the original request — a sign of a bug in our own error
   * handling code. The behavior we enforce (always echo back the original state
   * unchanged) IS required by RFC 6749 Section 4.1.2.1; the warning is just our
   * own diagnostic on top.
   *
  ...
  /**
   * CUSTOM PROXY DETAIL: Unhandled OAuth2 Endpoint Fallback
   *
   * Safety mechanism inside our proxy for catching errors on unknown OAuth2
   * endpoints not explicitly handled by the filter. Not part of any RFC — it
   * exists to prevent silent failures if new endpoints are added without a handler.
   *
   * Implementation: oauth2-exception.filter.ts (lines 58-77 - else branch in catch method)
   *
   * Known endpoints handled:
   * - /oauth2/authorize, /oauth2/continue (handleAuthorizationError)
   * - /oauth2/token (handleTokenError)
   * - /oauth2/store, /oauth2/info (handleInternalError)
   */
  describe('Custom Proxy Detail - Unhandled OAuth2 Endpoint', () => {
  ...
  /**
   * CUSTOM PROXY DETAIL: OAuth2Exception Metadata and Logging
   *
   * Custom OAuth2Exception class that carries metadata separate from client-visible errors.
   * The motivation aligns with RFC 6749 Section 5.2 ("error_description... MUST NOT include
   * characters outside the set %x20-21 / %x23-5B / %x5D-7E") and RFC 6819 Section 5.6
   * (don't leak internal diagnostics to clients) — but the OAuth2Exception class itself,
   * its metadata fields, and the logging structure are all internal to our proxy.
   *
   * Implementation:
  ...
  /**
   * CUSTOM PROXY DETAIL: Error Message Extraction Logic
   *
   * Flexible error message handling that normalizes various error formats coming from
   * inside our proxy (validation pipes, NestJS HttpExceptions, OAuth2Exception) into
   * RFC-conformant OAuth2 error responses (RFC 6749 Section 5.2 / Section 4.1.2.1).
   *
   * The output shape (`error`, `error_description`) is RFC-defined; the multi-format
   * input handling is custom to bridge NestJS exception conventions to that shape.
   *
   * Implementation:
   ...
   */
  describe('Custom Proxy Detail - Error Message Extraction', () => {
