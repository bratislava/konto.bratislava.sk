...
describe('Form access', () => {
  ...
  beforeAll(async () => {
    ...
    authUser1 = userFactory.createFoAuthUser()
    authUser2 = userFactory.createFoAuthUser()
    guestUser1 = userFactory.createGuestUser()
    guestUser2 = userFactory.createGuestUser()

    const commonIco = userFactory.generateRandomIco()
    const uniqueIco = userFactory.generateRandomIco()

    authUserWithIcoCommon1 = userFactory.createPoAuthUser({ ico: commonIco })
    authUserWithIcoCommon2 = userFactory.createPoAuthUser({ ico: commonIco })
    authUserWithIcoUnique = userFactory.createPoAuthUser({ ico: uniqueIco })
  ...
  describe('Error Handling & Invalid Requests', () => {
    ...
    it('should throw NotFoundException for non-existent formId', async () => {
      ...
      await expect(
        formAccessService.hasAccess(nonExistentFormId, authUser1.user),
  ...
  describe('Direct User Access', () => {
    it('should allow auth user to access their own form (Service & Guard)', async () => {
      ...
      expect(serviceAccess).toBe(FormAccessType.UserAccess)
  ...
  describe('Guest Identity Access', () => {
    it('should allow guest user to access their own form (Service & Guard)', async () => {
      ...
      expect(serviceAccess).toBe(FormAccessType.GuestIdentity)
  ...
  describe('ICO Access', () => {
    it('should allow auth user with ICO to access their own form as UserAccess (Service & Guard)', async () => {
      ...
      expect(serviceAccess).toBe(FormAccessType.UserAccess) // should return UserAccess since they're the owner
    ...
    it('should allow auth user with same ICO to access form as IcoAccess (Service & Guard)', async () => {
      ...
      expect(serviceAccess).toBe(FormAccessType.Ico)
  ...
  describe('Migration Access', () => {
    it('should allow auth user with valid migration to access guest form with checkMigrations=true (Service & Guard)', async () => {
      ...
      expect(serviceAccess).toBe(FormAccessType.Migration)
