...
const Toast = ({ className, ...props }: ToastProps<AppToastContent>) => {
  return (
    <ReactAriaToast
      ...
      className={cn(
        'pointer-events-auto flex w-full max-w-2xl items-center rounded text-white shadow-lg outline-none [view-transition-class:toast] sm:w-auto sm:min-w-83.5',
        className,
      )}
    />
  )
      ...
      {({ toast }) => (
        ...
        >
          <ToastContent className="flex min-w-0 flex-1 items-center">
            <Text
              slot="title"
              className="flex-1 px-4 py-3.5 text-left text-sm leading-5 font-normal"
            >
