    ...
    >
      <div className="bg-gray-50 pl-8 lg:pl-0">
        <div className="m-auto h-full w-full max-w-screen-lg flex-col justify-end gap-4 pt-6 lg:gap-6 lg:pt-14">
          <h1 className="text-h1 m-auto h-full w-full pt-4">{title}</h1>
          <TabList className="m-auto flex h-full w-full max-w-screen-lg gap-4 pt-6 lg:gap-6 lg:pt-14">
            {headerNavigationList.map((item) => (
              <Tab
                key={item.tag}
                id={item.tag}
                className="text-20 hover:text-20-semibold data-[selected]:text-20-semibold  w-full cursor-pointer py-4 transition-all hover:border-gray-700 data-[selected]:border-b-2 data-[selected]:border-gray-700"
              >
                {item.title}
              </Tab>
            ))}
