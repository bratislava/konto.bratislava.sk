 ...
 */
export const removeEmptySubtrees = <T extends JSONValue>(
  objOrArrayOrValue: T,
): T | null => {
...
export const generateTaxXml = (data: TaxFormData) => {
  const jsonObj = {
    'E-form': {
      $: {
        xmlns:
      ...
      Body: {
        ...
        ZakladneVyhlasenie: {
          ...
          PoskytujemSuhlasText:
            'Poskytujem súhlas na spracovanie osobných údajov',
          NeposkytujemSuhlas: 'false',
          NeposkytujemSuhlasText:
            'Neposkytujem súhlas na spracovanie osobných údajov',
        },
        Dorucenie: {
          AdresatPodania: {
