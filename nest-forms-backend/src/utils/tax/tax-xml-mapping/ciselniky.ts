...
} from '../shared-mapping/esbsCiselniky'
...
export function getCiselnikEntryByCondition<T extends CiselnikType>(
  ...
  const firstTruthyKey = (
    Object.keys(record) as Array<keyof typeof record>
  ).find((key) => record[key])
  if (!firstTruthyKey) {
    return null
  }
...
export const katastralneUzemie = (
  katastralneUzemieString: string | undefined,
) => {
  if (!katastralneUzemieString) {
    return null
  }

  return esbsKatastralneUzemiaCiselnik.find(
    ({ Name }) => Name === katastralneUzemieString,
  )
}

const obecFromKatastralneUzemie = (
  katastralneUzemieString: string | undefined,
) => {
  if (!katastralneUzemieString) {
    return null
  ...
  if (
    katastralneUzemieString === 'Nivy' ||
    katastralneUzemieString === 'Trnávka'
  ) {
    return getCiselnikEntryByCode(
      esbsBratislavaMestskaCastCiselnik,
      'SK0102529320',
    ) // Ružinov
  }
  ...
  return esbsBratislavaMestskaCastCiselnik.find(
    ({ Name }) =>
      Name === `Bratislava - mestská časť ${katastralneUzemieString}`,
  )
}
...
export const tituly = (value: string | undefined) => {
  ...
  const zaMenom = esbsTitulZaMenomCiselnik.find(
    ({ Name }) => fixValue(Name) === fixValue(value),
  )
