...
const InnerTooltip = ({ children, ...props }: InnerTooltipProps) => {
  return (
    <Tooltip
      {...props}
      className="m-0 flex w-fit max-w-[230px] min-w-[118px] flex-row justify-center rounded-sm border-0 bg-gray-700 px-3 py-2 text-size-p-small-r wrap-break-word text-white lg:max-w-[280px] lg:px-4 lg:py-3"
    >
      <OverlayArrow>
        {({ placement }) => {
          if (!placement) {
