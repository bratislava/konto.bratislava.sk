...
import { formatDatePdf } from '../shared-mapping/dates'
import { oddiel2Shared } from '../shared-mapping/oddiel2Shared'
  ...
  return generateCopies(mapping, (priznanie) => {
    const pozemky = priznanie.pozemky.map((pozemok, pozemokIndex) => {
      ...
      return {
        ...
        [`3_DatumVzniku${fixedIndex}`]: formatDatePdf(
          pozemok.datumVznikuDanovejPovinnosti,
        ),
        [`3_VymeraPozemku${fixedIndex}`]: formatDecimalPdf(
          pozemok.vymeraPozemku,
        ),
        [`3_DatumZaniku${fixedIndex}`]: formatDatePdf(
          pozemok.datumZanikuDanovejPovinnosti,
        ),
      }
    })
    ...
    return {
      ...
      '3_RodneCislo1Man': formatRodneCisloFirstPartPdf(
        priznanie.rodneCisloManzelaManzelky,
      ),
      '3_RodneCislo2Man': formatRodneCisloSecondPartPdf(
        priznanie.rodneCisloManzelaManzelky,
      ),
