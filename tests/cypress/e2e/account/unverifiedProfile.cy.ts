    ...
    .forEach((device) => {
      context(device, Cypress.env('resolution')[`${device}`], () => {
        ...
        it('2. Checking alert container.', () => {
          ...
          cy.get('[data-cy=alert-container]').find('[data-cy="alert-container-title"]').should('contain', 'Neoverený profil')
          cy.get('[data-cy=alert-container]').find('[data-cy="alert-container-content"]').should('contain', 'Vaša identita zatiaľ nebola overená voči štátnym registrom. Bratislavské konto môžete preto používať len v obmedzenom režime, bez možnosti odoslať elektronicky všetky žiadosti či využiť službu online platby dane z nehnuteľností.')
