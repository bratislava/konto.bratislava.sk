...
describe('EmailFormsSubservice', () => {
  ...
  describe('sendEmailForm', () => {
    ...
    beforeEach(() => {
      ...
      mailgunService.sendOloEmail.mockResolvedValue()
    })
    ...
    it('should successfully send emails using sendEmail function and update form state', async () => {
      ...
      expect(mailgunService.sendEmail).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          to: mockFormDefinitionWithSendEmail.email,
          template: mockFormDefinitionWithSendEmail.newSubmissionEmailTemplate,
      ...
      expect(mailgunService.sendEmail).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          to: userEmail,
          template: mockFormDefinitionWithSendEmail.userEmailTemplate,
          data: expect.objectContaining({
            formId: mockForm.id,
    ...
    it('should successfully send emails using sendOloEmail function and update form state', async () => {
      ...
      expect(mailgunService.sendOloEmail).toHaveBeenNthCalledWith(
        1,
        expect.objectContaining({
          to: mockFormDefinitionWithSendOloEmail.email,
          template:
            mockFormDefinitionWithSendOloEmail.newSubmissionEmailTemplate,
          data: expect.objectContaining({
      ...
      expect(mailgunService.sendOloEmail).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          to: userEmail,
          template: mockFormDefinitionWithSendOloEmail.userEmailTemplate,
          data: expect.objectContaining({
            formId: mockFormWithOloDefinition.id,
            firstName: userFirstName,
            slug: mockFormDefinitionWithSendOloEmail.slug,
    ...
    it('should extract email from form data when userEmail is null', async () => {
      ...
      expect(mockFormDefinitionWithSendEmail.extractEmail).toHaveBeenCalledWith(
    ...
    it('should extract name from form data when userFirstName is null', async () => {
      ...
      expect(mockFormDefinitionWithSendEmail.extractName).toHaveBeenCalledWith(
        mockForm.formDataJson,
      )
      expect(mailgunService.sendEmail).toHaveBeenNthCalledWith(
        2,
    ...
    it('should use null for firstName when both userFirstName and extractName are not available', async () => {
      mockFormDefinitionWithSendEmail.extractName = undefined
      await service.sendEmailForm(formId, userEmail, null)
    ...
    it('should log error but continue when sending confirmation email fails with sendOloEmail', async () => {
      ...
      mailgunService.sendOloEmail.mockResolvedValueOnce() // First call succeeds (department email)
      mailgunService.sendOloEmail.mockRejectedValueOnce(
    ...
    it('should log warning when no user email is available with sendEmail', async () => {
      jest
        .spyOn(mockFormDefinitionWithSendEmail, 'extractEmail')
    ...
    it('should log warning when no user email is available with sendOloEmail', async () => {
      ...
      prismaMock.forms.update.mockResolvedValue(mockFormWithOloDefinition)
      jest
        .spyOn(mockFormDefinitionWithSendOloEmail, 'extractEmail')
        // eslint-disable-next-line unicorn/no-useless-undefined
        .mockImplementation(() => undefined)
      const errorSpy = jest.spyOn(service['logger'], 'error')
