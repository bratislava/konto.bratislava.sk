...
test('F01 — stanovisko k investičnému zámeru', { tag: '@legacy' }, async ({ page }) => {
  ...
  await test.step('zodpovedný projektant — prázdny krok neprejde', async () => {
  ...
  await test.step('zodpovedný projektant', async () => {
    ...
    await fillIn(page, 'root_zodpovednyProjektant_priezvisko', data.zodpovednyProjektant.priezvisko)
