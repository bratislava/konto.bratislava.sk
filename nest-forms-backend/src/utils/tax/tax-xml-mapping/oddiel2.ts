...
import { esbsDruhPozemkuDaneCiselnik } from '../shared-mapping/esbsCiselniky'
import {
  Oddiel2PriznanieShared,
  oddiel2Shared,
} from '../shared-mapping/oddiel2Shared'
...
const mapPriznanie = (
  data: TaxFormData,
  priznanie: Oddiel2PriznanieShared,
  index: number,
) => ({
  ...
  RodneCisloManzelaAleboManzelky: formatRodneCisloXml(
  ...
  Pozemky: {
    Pozemok: priznanie.pozemky.map((pozemok, pozemokIndex) => ({
      ...
      DruhPozemku: getCiselnikEntryByCode(
        esbsDruhPozemkuDaneCiselnik,
        pozemok.druhPozemku,
      ),
...
export const oddiel2Xml = (data: TaxFormData) => {
  ...
  return {
    DanZPozemkov: {
      DanZPozemkovZaznam: mapping.map((priznanie, index) =>
