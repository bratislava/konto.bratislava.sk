...
import registraciaPredzahradky from 'schema-generator/definitions/registraciaPredzahradky'
...
/**
 * Temporary route for previewing the front garden registration form.
 */
export const getServerSideProps: GetServerSideProps<FormPageWrapperProps, Params> = async (ctx) => {
  ...
  const schema = JSON.parse(JSON.stringify(registraciaPredzahradky)) as {
    schema: RJSFSchema
    uiSchema: UiSchema
  }
